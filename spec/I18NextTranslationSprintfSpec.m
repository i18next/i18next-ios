    ...
    describe(@"using sprintf", ^{
        ...
        beforeEach(^{
            ...
            @{
              ...
              @"en": @{ @"translation": @{ } },
              @"en-US": @{ @"translation": @{
                                   @"interpolationTest1": @"The first 4 letters of the english alphabet are: %@, %@, %@ and %@",
        ...
        it(@"should replace passed in key/values", ^{
            NSString* value = [i18n t:@"interpolationTest1" options:@{ @"sprintf": I18NEXT_SPRINTF_ARGS(@"a", @"b", @"c", @"d") }];
        ...
        it(@"should recognize the sprintf syntax and automatically add the sprintf processor", ^{
            NSString* value = [i18n t:@"interpolationTest1", @"a", @"b", @"c", @"d"];
            expect(value).to.equal(@"The first 4 letters of the english alphabet are: a, b, c and d");
